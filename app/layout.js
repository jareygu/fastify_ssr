...
//   console.log(file, options.locals, options)
  ejs.renderFile(file, options.locals, options, function(err, html) {
    ...
    //   console.log("viewsDir", options.settings.views)
      if (layout.length > 0 && layout[0] === ".") {
...
function script(src, type = "text/javascript") {
  this.append(`<script type="${type}" src="${src}"></script>`);
}

// bound to stylesheets Block in renderFile
function stylesheet(href) {
  this.append(`<link rel="stylesheet" href="${href}">`);
}
